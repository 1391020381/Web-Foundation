 ...
 * `注意,并不是说所有的框都会产生FC,而是符合特定条件才会产生,只有产生了对应的FC后才会应用对应渲染规则。`
 5. BFC规则：
    ...
    * 总结BFC特点：
        * 内部box在垂直方向,一个接一个的放置
        * box的垂直方向有margin决定,属于同一个BFC的两个box间的margin会重叠
...
6.  IFC(Inline Formatting Context)
    * IFC即行内框产生的格式上下文
    * IFC规则
        * 在行内格式化上下文中
...
* 即不包含文本，保留空白符，margin/padding/border非0的行内元素，以及其他常规流中的内容(比如，图片inlineblocks 和 inline tables)，并且不是以换行结束的行框，必须被当作零高度行框对待
* 总的来说,它就是行内元素自身如何显示以及在框内如何摆放的渲染规则,这样描述应该更容易理解
7. 其它
    * 譬如常规流，浮动，绝对定位等区别
  ...
  * 譬如z-index的分层显示机制等
8. [CSS核心可视化格式模型](https://bbs.csdn.net/topics/340204423)
# js引擎解析过程
* [JavaScript深入系列](https://github.com/1391020381/Web-Foundation/tree/master/articles/JavaScript%E7%9B%B8%E5%85%B3/JavaScript%E6%B7%B1%E5%85%A5%E7%B3%BB%E5%88%97)
# 回收机制
...
* 常用的两种垃圾回收规则是：
    * 标记清除
    * 引用计数
* JavaScript引擎基础GC方案(simple GC) ：mark and sweep (标记清除)
    * 遍历所有可访问的对象
    * 回收已不可访问的对象
* 譬如：(出自javascript高程)
  ...
  * 关于引用计数，简单点理解：跟踪记录每个值被引用的次数，当一个值被引用时，次数+1，减持时-1，下次垃圾回收器会回收次数为0的值的内存（当然了，容易出循环引用的bug）       


* [V8 内存浅析](https://zhuanlan.zhihu.com/p/33816534)
# 其它
* [跨域](https://github.com/1391020381/Web-Foundation/tree/master/articles/HTTP%E3%80%81TCP%E3%80%81IP/Ajax%E8%B7%A8%E5%9F%9F)
* [web安全](https://github.com/1391020381/Web-Foundation/tree/master/articles/HTTP%E3%80%81TCP%E3%80%81IP/Web%E5%AE%89%E5%85%A8)
* viewport
* node
